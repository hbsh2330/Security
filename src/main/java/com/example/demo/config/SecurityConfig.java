...
//        );
        ...
        http.authorizeHttpRequests(//요청 uri와 로그인한 계정의 권한이 로그인한 계정과 일치하는지? // 권한체크
                authorize->{
                    authorize.requestMatchers("/js/**","/css/**","/images/**","/templates").permitAll(); //js를 시큐리티 코드가 권한 없이 모두 허용
                    authorize.requestMatchers("/", "login", "/user/**").permitAll(); // "/"와 "login"으로 들어온 녀석은 권한체크없이 프리패스
//                    authorize.requestMatchers("/join").hasRole("ANONYMOUS"); // 지정을 안해줬을 때 기본값으로 ANONYMOUS을 스프링 시큐리티가 부여해준다.
//                    authorize.requestMatchers("/", "/login", "/user/certification").permitAll();
